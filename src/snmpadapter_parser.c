...
int snmpadapter_extract_snmp_get_params(cJSON *request, snmpadapter_record** psnmp_params)
{
    ...
    *psnmp_params = snmp_params; //allocate memory and pass to caller to free
    ...
    SnmpAdapterPrint("[SNMPADAPTER] extract_snmp_get_params() :snmp_params->type : %d\n", snmp_params->type);
    ...
    SnmpAdapterPrint("[SNMPADAPTER] extract_snmp_get_params() :snmp_params->u.get->count : %d\n", snmp_params->u.get->count);
    ...
    for (i = 0; i < oidCount; i++)
    {
        ...
        SnmpAdapterPrint("[SNMPADAPTER] extract_snmp_get_params() :snmp_params->u.get->oid[%d] : %s\n", i, snmp_params->u.get->oid[i]);
    }
